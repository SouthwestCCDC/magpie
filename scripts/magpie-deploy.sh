...
generate_env_file() {
    ...
    # Build Caddy environment variables based on TLS mode
    local caddy_site_address
    local caddy_logging
    local caddy_admin
    local caddy_security_headers
    local caddy_tls_config
    ...
    level INFO
            ...
            # No HSTS for HTTP-only mode, but include other security headers
            caddy_security_headers='X-Content-Type-Options "nosniff"
...
-Server'
    ...
    level INFO
            ...
            caddy_admin="admin off"
            caddy_security_headers='Strict-Transport-Security "max-age=2592000; includeSubDomains"
X-Content-Type-Options "nosniff"
X-Frame-Options "DENY"
X-XSS-Protection "1; mode=block"
Referrer-Policy "strict-origin-when-cross-origin"
-Server'
            caddy_tls_config=""  # Automatic TLS (default)
            ;;
        manual)
...
Referrer-Policy "strict-origin-when-cross-origin"
-Server'
            caddy_tls_config="tls /etc/caddy/tls/cert.pem /etc/caddy/tls/key.pem"
            ;;
    esac
...
# Caddy environment variables for consolidated Caddyfile (issue #232)
MAGPIE_SITE_ADDRESS=${caddy_site_address}
MAGPIE_ENABLE_LOGGING=${caddy_logging}
MAGPIE_DISABLE_ADMIN=${caddy_admin}
MAGPIE_PROD_SECURITY_HEADERS=${caddy_security_headers}
MAGPIE_TLS_CONFIG=${caddy_tls_config}
